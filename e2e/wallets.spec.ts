import { test, expect, navigate, waitForAnimations, waitForStableBox } from "./fixture.ts";
...
test.describe("wallets", () => {
  ...
  test("lists every wallet with its balance and which one is the default", async ({ app, seed }) => {
    for (const wallet of seed.wallets) {
      await expect(app.locator(".wallet-tiles > .wallet-tile").filter({ hasText: wallet.name })).toHaveCount(1);
    }
    await expect(app.getByText("กระเป๋าหลัก")).toBeVisible();
  });

  test("opens a wallet's actions without a page error", async ({ app }) => {
    const card = app.locator(".wallet-tiles > .wallet-tile").filter({ hasText: "กระเป๋าย่อย" }).first();
