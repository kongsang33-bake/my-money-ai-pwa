...
test.describe("split review", () => {
  ...
  test("moves the debt when the share changes, without changing the bill", async ({ page }) => {
    ...
    await expect(page.locator(".draft-effects")).toContainText("163");
    await expect(page.locator(".draft-effects")).toContainText("100");
  ...
  test("takes a headcount a party actually needs, past the old ÷2 ÷3 ÷4", async ({ page }) => {
    ...
    await expect(page.locator(".draft-effects")).toContainText("1,000");
  });
  ...
  test("breaks a party into one debt per person before it is saved", async ({ page }) => {
    ...
    await expect(page.locator(".draft-effects")).toContainText("1,200");
  });
  ...
  test("divides what is left when the user pins their own share", async ({ page }) => {
    ...
    await expect(page.locator(".draft-effects")).toContainText("500");
  });
  ...
  test("names the people it is about to create debts for", async ({ page }) => {
    ...
    await expect(page.locator(".draft-attention")).toContainText("ชื่อใหม่ อ้อน, แบงค์");
  });
  ...
  test("offers a credit card as the thing that paid, not just a wallet", async ({ page }) => {
    ...
    const result = page.locator(".draft-effects");
  ...
  test("offers someone who already owes the user, and takes the bill off what they owe", async ({ page }) => {
    ...
    // Down, not up: the balance falls by what the bill cost.
    const result = page.locator(".draft-effects");
    await expect(result).toContainText("เอก");
    await expect(result).toContainText("ลดลง");
    await expect(result).toContainText("163");
  });
