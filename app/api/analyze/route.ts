...
const itemSchema = {
  ...
  properties: {
    ...
    paid_with_card: {
      type: "string",
      description: "ใครเป็นคนออกเงินก้อนนี้แทนกระเป๋าของผู้ใช้ - ชื่อบัตรเครดิต หรือชื่อคนที่สำรองจ่าย/ออกให้ก่อน ใช้กับ transaction_type เป็น personal_expense, split_half หรือ lend ถ้าผู้ใช้จ่ายเองด้วยเงินในกระเป๋า ให้ส่งค่าว่าง",
    },
    partner_share: {
      type: "number",
      minimum: 0,
      description: "ยอดที่อีกฝ่ายต้องคืน ใช้เฉพาะ transaction_type เป็น split_half และมีคนเกี่ยวข้องคนเดียว ถ้าหารครึ่งเท่ากันหรือไม่ได้ระบุสัดส่วน ให้ส่ง 0 แล้วแอพจะหารครึ่งเอง",
    },
    user_share: {
      type: "number",
      minimum: 0,
      description: "ยอดที่ผู้ใช้ออกเอง ใช้เฉพาะ transaction_type เป็น split_half ที่ระบุชื่อคนหลายคน ถ้าไม่ได้ระบุว่าผู้ใช้ออกเท่าไร ให้ส่ง 0 แล้วแอพจะหารเท่ากันเอง",
    },
    ambiguous: {
      type: "boolean",
  ...
  required: ["title", "category", "amount", "transaction_type", "debtor_name", "date", "note", "wallet_id", "transfer_to_wallet_id", "paid_with_card", "partner_share", "user_share", "ambiguous"],
...
function buildPrompt(
  ...
  return [
    ...
    ...(hasImages
      ...
      : []),
    "กติกาข้อความที่มีหลายรายการ:",
    "- ข้อความเดียวอาจมีหลายเหตุการณ์ ให้แยกเป็นคนละรายการเสมอ อย่ายุบรวมกัน เช่น บิลสองใบในมื้อเดียวกัน (โปรแรก/โปรที่สอง, บิล 1/บิล 2) คือสองรายการ เพราะคนที่จ่ายและคนที่หารอาจไม่เหมือนกัน",
    "- ทุกยอดที่ปรากฏในข้อความต้องมีที่อยู่ในผลลัพธ์ ถ้ามีตัวเลขที่ยังไม่ได้ใช้ ให้ทบทวนว่าตกรายการไปหรือไม่",
    "",
    ...
    "- ห้ามใช้ borrow กับกรณี \"เพื่อนออกให้ก่อน / สำรองจ่ายให้ / เขาจ่ายไปก่อน\" เพราะเงินสดไม่ได้เข้ากระเป๋าผู้ใช้เลย มีแค่ของที่ซื้อกับหนี้ที่ติดเขา — ให้ใช้ transaction_type ตามปกติของค่าใช้จ่ายนั้น (personal_expense ถ้าเป็นส่วนของผู้ใช้เอง) แล้วใส่ชื่อคนที่ออกให้ก่อนใน paid_with_card แทน",
    ...
    "",
    "กติกา paid_with_card (คนอื่นหรือบัตรเป็นคนออกเงินให้):",
    "- split_half และ lend จ่ายด้วยบัตรเครดิตได้ ให้ใส่ชื่อบัตรลงใน paid_with_card โดยเลือกจากรายชื่อหนี้ของฉันด้านล่างให้ตรงที่สุด เช่น \"ข้าวเย็น 163 จ่ายผ่าน spay หารกับจูน\" -> transaction_type = split_half, debtor_name = จูน, paid_with_card = ชื่อบัตรที่ตรงกับ spay",
    "- ห้ามเปลี่ยน transaction_type เป็น card_charge เพราะเห็นว่าจ่ายด้วยบัตร ถ้ารายการนั้นมีการหารหรือออกให้คนอื่น — card_charge ใช้เฉพาะตอนจ่ายเองคนเดียวเต็มจำนวน",
    "- ใช้กับคนได้ด้วย ไม่ใช่แค่บัตร: ถ้าคนอื่นออกเงินให้ก่อนแล้วผู้ใช้ติดเขาอยู่ ให้ใส่ชื่อคนนั้นใน paid_with_card เช่น \"บิลแรก 778 หาร 2 กับอ้อน อ้อนออกก่อน\" -> ส่วนของผู้ใช้คือ 389 ให้ส่ง transaction_type = personal_expense, amount = 389, paid_with_card = อ้อน (ไม่ใช่ amount 778 เพราะอีก 389 เป็นส่วนของอ้อนเองที่เขาจ่ายของเขาเอง)",
    "- ชื่อใน paid_with_card เลือกได้จากทั้งรายชื่อหนี้ของฉันและรายชื่อลูกหนี้ (คนที่ติดเรา) ด้านล่าง ถ้าคนที่ออกให้ก่อนเป็นคนที่ติดเราอยู่ แอพจะหักยอดบิลนี้ออกจากยอดที่เขาติดเราให้เอง — ให้ส่งแค่รายการค่าใช้จ่ายรายการเดียว ห้ามเติมรายการ debt_repayment มาเองเพื่อหักลบ เช่น \"ข้าวเที่ยง 70 จูนออกให้\" เมื่อจูนอยู่ในรายชื่อลูกหนี้ -> transaction_type = personal_expense, amount = 70, paid_with_card = จูน (ไม่ใช่สองรายการ)",
    "- ถ้าชื่อที่พูดถึงไม่มีอยู่ในรายชื่อทั้งสองด้านล่าง หรือจ่ายด้วยเงินในกระเป๋า ให้ส่ง paid_with_card เป็นค่าว่าง",
    "",
    "กติกา partner_share (หารไม่เท่ากัน):",
    "- ใช้เฉพาะ split_half และให้เป็น \"ยอดที่อีกฝ่ายต้องคืน\" เป็นบาท ไม่ใช่ยอดของผู้ใช้เอง",
    "- หารครึ่งปกติ/คนละครึ่ง/ไม่ได้ระบุสัดส่วน ให้ส่ง 0",
    "- ถ้าระบุยอดของอีกฝ่ายชัดเจน เช่น \"หารกับจูน จูนจ่าย 100\" ให้ใส่ 100",
    "- ถ้าระบุจำนวนคนที่หารเท่า ๆ กัน N คน (นับผู้ใช้ด้วย) และผู้ใช้เป็นคนจ่ายบิลทั้งหมด ให้ partner_share = ยอดทั้งหมด ลบด้วย (ยอดทั้งหมด ÷ N) เพราะส่วนที่เหลือคือยอดที่คนอื่นต้องคืนผู้ใช้ เช่น ยอด 1200 หารกัน 6 คน ให้ partner_share = 1000 (ส่วนของผู้ใช้เองคือ 200)",
    "",
    "กติกา debtor_name:",
    ...
    "- ถ้าข้อความบอกชื่อคนที่ร่วมหาร/ที่เราออกให้มากกว่าหนึ่งคน ให้ใส่ชื่อทุกคนใน debtor_name คั่นด้วยเครื่องหมายจุลภาค (,) เช่น \"ค่าเบียร์ 1500 หาร 5 คน มีอ้อน แบงค์ วิน พี่พัก ผม\" -> debtor_name = \"อ้อน, แบงค์, วิน, พี่พัก\" (ห้ามใส่ผู้ใช้เองหรือคำว่า ผม/ฉัน/เรา ลงไปในรายชื่อ และห้ามยุบเป็นคำว่า เพื่อน เมื่อรู้ชื่อจริงแล้ว)",
    "- เมื่อใส่หลายชื่อ ให้ส่ง partner_share = 0 เสมอ แอพจะหารยอดให้เท่า ๆ กันตามจำนวนชื่อเอง",
    "",
    "กติกา user_share (ผู้ใช้ออกเองไม่เท่าคนอื่น):",
    "- ใช้เฉพาะ split_half ที่ระบุชื่อคนหลายคน และข้อความบอกชัดว่าผู้ใช้ออกเงินเท่าไร เช่น \"ค่าเบียร์ 1000 ผมออก 500 ส่วนที่เหลือหาร 3 คน มีอ้อน แบงค์ วิน\" -> amount = 1000, debtor_name = \"อ้อน, แบงค์, วิน\", user_share = 500",
    "- ห้ามคำนวณยอดของแต่ละคนเอง ให้ส่งแค่ยอดที่ผู้ใช้ออก แล้วแอพจะหารส่วนที่เหลือให้เท่า ๆ กันและปัดเศษให้ลงตัวพอดีกับบิล",
    "- ถ้าไม่ได้บอกว่าผู้ใช้ออกเท่าไร ให้ส่ง 0",
    ...
    "",
    "กติกาการเคลียร์ยอดกัน:",
    "- ถ้าข้อความบอกว่าจ่ายคืน/เคลียร์ยอดกับใครไปแล้ว ให้บันทึกเฉพาะเงินที่เคลื่อนจริงในทิศทางที่ถูก: ผู้ใช้จ่ายให้เขา = debt_payment, เขาจ่ายคืนผู้ใช้ = debt_repayment โดยใช้ยอดที่ระบุไว้ในข้อความ",
    "- ห้ามสร้างรายการหักกลบหนี้สองฝั่งเอง แอพมีปุ่มเคลียร์ยอดของตัวเองสำหรับล้างยอดค้างทั้งสองทาง",
    "",
    "กติกา ambiguous:",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    }, { timeoutMs: images.length > 0 ? GEMINI_IMAGE_TIMEOUT_MS : GEMINI_TEXT_TIMEOUT_MS, thinking: "minimal" });
  } catch (error) {
